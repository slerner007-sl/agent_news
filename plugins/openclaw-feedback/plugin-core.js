...
const DEFAULT_PROJECT_ROOT = existsSync(resolve(REPO_ROOT, "src/agent_news"))
  ? REPO_ROOT
  : "/home/user1/gosb_bot";
const SENDER_PY_PATH = resolve(DEFAULT_PROJECT_ROOT, "src/agent_news/sender.py");
...
const KNOWLEDGE_FILE_READER_PATH = resolve(DEFAULT_PROJECT_ROOT, "src/agent_news/knowledge_file_reader.py");
...
function extractFileText(filePath, pluginConfig = {}) {
  const path = cleanText(filePath);
  if (!path || !existsSync(path)) return { text: "", error: "file_not_found" };
  const readerPath = cleanText(pluginConfig.knowledgeFileReaderPath) || KNOWLEDGE_FILE_READER_PATH;
  try {
    const raw = execFileSync("python3", [readerPath, path], {
      encoding: "utf8",
      timeout: 30_000,
...
function saveKnowledgePayload({ event, ctx, pluginConfig, kind }) {
  ...
  for (const media of mediaEntries) {
    const extracted = extractFileText(media.path, pluginConfig);
